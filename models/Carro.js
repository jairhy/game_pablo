...
function des_text(text, x, y, cor, font) {
  des.fillStyle = cor;
  des.font      = font;
...
class Laser {
  ...
  atirar(nave) {
    if (this.ativo || nave.vida <= 0) return false;
  ...
  colid(meteoro) {
    ...
    return (
      dist(this.x, this.y, meteoro.x + meteoro.w / 2, meteoro.y + meteoro.h / 2) <
...
class Explosion {
  ...
  acertou_nave(nave) {
    ...
    return (
      dist(nave.x + nave.w / 2, nave.y + nave.h / 2, this.cx, this.cy) < this.raio_dano
    );
  }
...
class Coracao extends Obj {
  ...
  recomeca() {
    ...
    this.y  = canvas.height / 2 - this.h / 2 + (Math.random() * variacao * 2 - variacao);
  ...
  mov_car() {
    ...
    this.x -= this.vel;
    if (this.x < -100) {
      if (typeof fase !== "undefined" && fase >= 3) this.ativo = false;
      else this.recomeca();
    }
  }
...
class Boss extends Obj {
  ...
  atirar() {
    if (!this.ativo || this.morreu || this.x > canvas.width - this.w - 20) return;
  ...
  colid_laser_player_multi(naves) {
    ...
    for (const l of this.lasers) {
      ...
      for (const nave of naves) {
        ...
        if (dist(nave.x + nave.w / 2, nave.y + nave.h / 2, l.x, l.y) < l.raio + 60) {
          l.ativo = false;
          nave.vida -= 1;
          batida.play();
  ...
  receber_laser(laser_player) {
    ...
    if (dist(laser_player.x, laser_player.y, this.x + this.w / 2, this.y + this.h / 2) <
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        this.fragmentos.push({
          ...
          vy:     Math.sin(angulo) * vel * (0.6 + Math.random() * 0.8) - Math.random() * 3,
          rot:    0,
  ...
  des_morte() {
    ...
    for (const f of this.fragmentos) {
      ...
      des.rotate(f.rot);
      if (imgOk)
        des.drawImage(this.img, f.sx, f.sy, f.sw, f.sh, -f.w / 2, -f.h / 2, f.w, f.h);
...
class BossMessage {
  ...
  des_carro() {
    ...
    des.textAlign   = "center";
    des.fillText("John Cracker is coming...", canvas.width / 2, canvas.height / 2 + 15);
    des.shadowBlur  = 0;
...
const CORES_CONFETE = [
  "#FFD700", "#FF69B4", "#00CFFF",
  "#7FFF00", "#FF6347", "#DA70D6",
  "#FFA500", "#FFFFFF",
];
