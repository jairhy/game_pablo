...
const nave1 = new NavePlayer(330, 300, 220, 240, "./img/pingu67.png", 1);
const nave2 = new NavePlayer(330, 650, 220, 240, "./img/nave2.png",   2);
const naves = [nave1, nave2];

const meteoros = [
  new Meteoro(canvas.width + 100, Math.random() * (canvas.height - 745), 745, 745, "./img/meteoro.png"),
  new Meteoro(canvas.width + 500, Math.random() * (canvas.height - 745), 745, 745, "./img/meteoro.png"),
  new Meteoro(canvas.width + 900, Math.random() * (canvas.height - 745), 745, 745, "./img/meteoro.png"),
];
...
const boss        = new Boss(0, 0, 600, 600, "./img/boss.png");
const boss_message = new BossMessage();
let   boss_apareceu          = false;
let   boss_lasers_count_anterior = 0;
let   boss_vida_anterior         = 60;

const laser1        = new Laser("#00FFFF");
const laser2        = new Laser("#FFFF00");
const lasers_player = [laser1, laser2];

const notificacoes  = new Notificacao();
const tela_vitoria  = new TelaVitoria();
...
const batida = new Audio("./img/batida.mp3");
batida.volume = 0.5;

const som_laser = new Audio("./img/laser.mp3");
som_laser.volume = 0.4;

const som_explosao = new Audio("./img/explosion.mp3");
som_explosao.volume = 0.6;

const som_vitoria = new Audio("./img/vitoria.mp3");
som_vitoria.volume = 0.8;
som_vitoria.loop   = true;

const som_tiro_boss = new Audio("./img/laser_boss.mp3");
som_tiro_boss.volume = 0.5;

const som_dano_boss = new Audio("./img/danoBoss.mp3");
som_dano_boss.volume = 0.5;

const som_game_over = new Audio("./img/gameOver.mp3");
som_game_over.volume = 0.9;

const som_boss_aparece = new Audio("./img/inicio.mp3");
...
function reiniciar() {
  nave1.x = 330; nave1.y = 300; nave1.vida = 10; nave1.dir = 0;
  nave2.x = 330; nave2.y = 650; nave2.vida = 10; nave2.dir = 0;
  ...
  explosoes.forEach(e => { e.ativo = false; e.particulas = []; });
...
document.addEventListener("keydown", e => {
  ...
  if (e.key === " ") {
    e.preventDefault();
    if (laser_cooldown2 <= 0 && laser2.atirar(nave2)) {
      som_laser.currentTime = 0;
      som_laser.play();
...
canvas.addEventListener("click", e => {
  ...
  const px   = canvas.width / 2 - 450;
  const py   = canvas.height / 2 - 210;
  const bx   = canvas.width / 2 - 200;
  const by   = py + 385;
  if (mx >= bx && mx <= bx + 400 && my >= by && my <= by + 55) reiniciar();
});
...
function mover_naves() {
  nave1.dir = teclas["w"] || teclas["W"] ? -10 : teclas["s"] || teclas["S"] ? 10 : 0;
...
function ver_fase() {
  ...
  if (fase === 3 && !boss_apareceu) {
    ...
    meteoros.forEach(m => { m.x = 99999; m.vel = 0; });
    coracao.ativo = false;
  ...
  for (let i = 0; i < lasers_player.length; i++) {
    ...
    for (let j = 0; j < meteoros.length; j++) {
      if (lsr.colid(meteoros[j])) {
        lsr.ativo = false;
        explosoes[j].explodir(meteoros[j].x + meteoros[j].w / 2, meteoros[j].y + meteoros[j].h / 2);
...
function dano_explosao() {
  if (dano_cooldown > 0) { dano_cooldown--; return; }
...
function atualizar_jogo() {
  ...
  if (boss.ativo && boss.vida < boss_vida_anterior) tocar_som(som_dano_boss);
  ...
  if (boss.lasers.length > boss_lasers_count_anterior) tocar_som(som_tiro_boss);
...
function desenhar_cooldown_laser(barra_x, barra_y, cooldown, cor_pronto, label) {
  const bw    = 220, bh = 16, raio = bh / 2;
  ...
  des.textAlign    = label_dir ? "right" : "left";
  const label_x    = label_dir ? barra_x - 8 : barra_x + bw + 8;
  des.fillText(label + (pronto ? " ▶" : "..."), label_x, barra_y + bh / 2);
...
function desenhar_hud_jogador(nave, lado, cooldown, cor_laser) {
  ...
  if (vida_pct > 0.05) {
    ...
    des.beginPath();
    des.roundRect(barra_x + 3, barra_y + 3, bw * vida_pct - 6, bh / 3, raio / 2);
    des.fill();
  }
  ...
  desenhar_cooldown_laser(barra_x, barra_y + bh + 6, cooldown, cor_laser, lado === 1 ? "F" : "Espaço");
}
...
function desenhar_controles() {
  ...
  des.textAlign   = "right";
  des.fillText("P2: ↑↓ mover | Espaço atirar", canvas.width - 20, canvas.height - 20);
  des.restore();
}

function desenhar_game_over() {
  des.fillStyle = "rgba(0,0,0,0.7)";
  des.fillRect(0, 0, canvas.width, canvas.height);
  t1.des_text("Não sobra nada",          canvas.width / 2 - 230, canvas.height / 2 - 40, "red",   "bold 90px Arial");
  t2.des_text("Pontuação Final: " + pontos_compartilhados, canvas.width / 2 - 210, canvas.height / 2 + 60, "white", "bold 42px Arial");
}
